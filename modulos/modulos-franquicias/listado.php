<?php 
    ...
    $modulos= $conexion->query("SELECT * FROM modulos WHERE fkdepartamento = 3 ORDER BY nivel_mod ASC");
    if(!$modulos)
        echo $conexion->error;
?>
                                        ...
                                        <tbody>
                                            ...
                                            while($row = $modulos->fetch_array())
                                            {
                                                ...
                                                <tr class="odd gradeX" id="fila<?=$i?>">
                                                    ...
                                                    <td><?= $row['nivel_mod'] ?></td>
                                                    <td>
                                                        <a href="../test-franquicias/listado.php?id=<?= $row['idmodulo'] ?>" class="tooltip-success preguntas" data-rel="tooltip" title="Preguntas <?= $row['nombre'] ?>" style="text-decoration:none;"> 
                                                        ...
                                                        &nbsp;
                                                        <a href="" data-id="<?= $row['idmodulo'] ?>"  class="moduloFranquiciasEliminar" data-fila="<?=$i?>" data-rel="tooltip" title="Eliminar <?= $row['nombre'] ?>" objeto="<?= $row['idmodulo'] ?>"> 
